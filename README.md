# GPUStack 与 vLLM-Router 动态模型实例负载均衡调度工具 (`gpu-vllm-router`)
...
`gpu-vllm-router` 是一个专为 **[GPUStack](https://github.com/gpustack/gpustack)** AI 集群与 **[vLLM-Router](https://github.com/vllm-project/router)**（vLLM 官方高性能请求路由器）打造的自动化服务发现与动态负载均衡桥接系统。

系统支持 **全集群多模型自动同步（无需每次指定固定的模型名称）** 与 **单模型精准定向路由** 两种模式：
- **全集群多模型自动同步（推荐默认）**：无需指定模型名称，自动发现并同步 GPUStack 上所有处于运行中（`state == "running"`）的模型与实例，统一对外暴露 OpenAI 兼容端点，根据请求中的 `model` 字段自动分发路由，动态聚合 `/v1/models` 列表。
- **单模型定向路由**：通过 `-model <name>` 锁定特定模型，为关键高吞吐模型独立分配专用负载均衡器。
